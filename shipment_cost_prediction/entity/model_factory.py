...
def evaluate_classification_model(model_list: list, 
    ...
    MetricInfoArtifact = namedtuple("MetricInfo",
                                ["model_name", "model_object", "train_r2", "train_r2", "train_accuracy",
    ...
    try:
        ...
        for model in model_list:
            ...
            logging.info(f"{'>>' * 30} r2 Score {'<<' * 30}")
            logging.info(f"Diff test train accuracy: [{diff_test_train_acc}].")
            logging.info(f"Train root mean squared error: [{train_r2}].")
            logging.info(f"Test root mean squared error: [{test_r2}].")
